...
import { deleteMemoEmbedding, updateSemanticGroupsAsync } from "@/lib/embeddings";
...
export const useMemoActionHandlers = ({ memo, onEdit, setDeleteDialogOpen, setRemoveTasksDialogOpen }: UseMemoActionHandlersOptions) => {
  ...
  const confirmDeleteMemo = useCallback(async () => {
    ...
    // Also delete the embedding from Supabase and update semantic groups
    deleteMemoEmbedding(memo.name)
      .then(() => updateSemanticGroupsAsync())
      .catch((error) => {
        console.error("Failed to delete memo embedding:", error);
      });
