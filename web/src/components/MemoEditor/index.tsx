...
import { saveMemoEmbedding } from "@/lib/embeddings";
...
const MemoEditorImpl: React.FC<MemoEditorProps> = ({
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Save embedding to Supabase asynchronously (non-blocking)
      console.log("Saving embedding for memo:", result.memoName);
      saveMemoEmbedding(result.memoName, state.content)
        .then(() => {
          console.log("Embedding saved successfully");
        })
        .catch((error) => {
          console.error("Failed to save memo embedding:", error);
          toast.error("Failed to save embedding: " + (error instanceof Error ? error.message : String(error)));
        });
